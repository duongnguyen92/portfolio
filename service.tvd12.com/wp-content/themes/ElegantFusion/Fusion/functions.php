...
function et_fusion_load_scripts_styles(){
	...
	wp_enqueue_script( 'jquery-ui', $template_dir . '/js/jquery-ui.js', array( 'jquery' ), '1.0', true );
...
function wp_exist_sell( $value1, $value2, $post_meta1, $post_meta2 ,$post_type) {
	global $wpdb;
	$query = "SELECT ID FROM wp_posts as main INNER JOIN wp_postmeta as postmeta ON main.ID = postmeta.post_id WHERE postmeta.meta_key = '" . $post_meta1 . "' AND postmeta.meta_value = '$value1' AND postmeta.meta_key = '" . $post_meta2 ."' AND post.meta_value = '". $value2 ."' AND main.post_type = '$post_type' ";
	$return = $wpdb->get_results( "SELECT ID FROM wp_posts as main INNER JOIN wp_postmeta as postmeta ON main.ID = postmeta.post_id WHERE postmeta.meta_key = '" . $post_meta1 . "' AND postmeta.meta_value = '$value1' ", 'ARRAY_N' );
	echo $return;
	die("ccc");
	if( empty( $return ) ) {
		return false;
	} else {
		// $result = $wpdb->get_row( "SELECT meta_value FROM wp_postmeta as postmeta INNER JOIN wp_posts as main ON main.ID = postmeta.post_id WHERE postmeta.meta_key = '" . $post_meta2 . "' AND postmeta.post_id = '$return' ", 'ARRAY_N' );
		return true;
	}
}

function wp_exist_post_by_meta( $value, $post_meta, $post_type) {
	global $wpdb;
	$return = $wpdb->get_row( "SELECT ID FROM wp_posts as main INNER JOIN wp_postmeta as postmeta ON main.ID = postmeta.post_id WHERE postmeta.meta_key = '" . $post_meta . "' AND postmeta.meta_value = '$value' AND main.post_type = '$post_type' ", 'ARRAY_N' );
	if( empty( $return ) ) {
		return false;
	} else {
		return true;
...
function prefix_ajax_add_foobar() {
	if (isset($_POST['post_title'])){
		$post_save = array();
		$post_save['post_title'] = $_POST['post_title'];
		$post_save['post_content'] = $_POST['post_content'];
		$post_save['post_status'] = 'publish';
		$post_save['post_type'] = 'sanpham';
		
		// Insert the post into the database
		if( wp_exist_post_by_title( $_POST['post_title'], "sanpham" ) ) {
			?>
				<p class="error-meassage">Sản phẩm đã tồn tại! </p>
			<?php
		} else { 
			$post_id = wp_insert_post( $post_save );
			if(!isset($_POST['ma_sp']) || $_POST['ma_sp'] =='')
			{
				add_post_meta( $post_id, 'ma_sp', $post_id );
			}else{
				add_post_meta( $post_id, 'ma_sp', $_POST['ma_sp'] );
			}
			if($post_id) {
				?>
					<p class="success-meassage">Thêm sản phẩm thành công! </p>
				<?php
			}
		}
					...
					<?php 
						if ($posts) {
							foreach ( $posts as $post ) { ?>
								<tr>
									<?php $postID = $post->ID; ?>
									<td><?php echo get_post_meta( $postID, 'ma_sp', true ) ?></td>
...
add_action( 'wp_ajax_add_customer', 'prefix_ajax_add_customer' );
add_action( 'wp_ajax_nopriv_add_customer', 'prefix_ajax_add_customer' );

function prefix_ajax_add_customer() {
	if (isset($_POST['post_title'])){
		$post_save = array();
		$post_save['post_title'] = $_POST['post_title'];
		$post_save['ma_kh'] = $_POST['ma_kh'];
		$post_save['post_status'] = 'publish';
		$post_save['post_type'] = 'khachhang';
		
		// Insert the post into the database
		if( wp_exist_post_by_meta( $_POST['ma_kh'], "ma_kh", "khachhang" ) ) {
			?>
				<p class="error-meassage">Khách hàng đã tồn tại! </p>
			<?php
		} 
		else { 
			$post_id = wp_insert_post( $post_save );
			if(!isset($_POST['ma_kh']) || $_POST['ma_kh'] =='')
			{
				add_post_meta( $post_id, 'ma_kh', $post_id );
			}else{
				add_post_meta( $post_id, 'ma_kh', $_POST['ma_kh'] );
			}
			add_post_meta( $post_id, 'diachi', $_POST['diachi'] );
			add_post_meta( $post_id, 'ma_thue', $_POST['ma_thue'] );
			if($post_id) {
				?>
				<p class="success-meassage">Thêm khách hàng thành công! </p>
			<?php
			}
		}
...
?>
		...
		<table class="table table-condensed table-striped list-product">
			<thead>
				<tr>
					<th class="col-sm-1">Mã khách hàng</th>
					...
					<?php 
						if ($posts) {
							foreach ( $posts as $post ) { ?>
								<tr>
									<?php $postID = $post->ID; ?>
									<td><?php echo get_post_meta( $postID, 'ma_kh', true ) ?></td>
									<td><?php echo $post->post_title; ?></td>
									<td><?php echo get_post_meta( $postID, 'diachi', true ) ?></td>
								</tr>
								<?php
							  }
							}
...
add_action( 'wp_ajax_add_banhang', 'prefix_ajax_add_banhang' );
add_action( 'wp_ajax_nopriv_add_banhang', 'prefix_ajax_add_banhang' );

function prefix_ajax_add_banhang() {
	if (isset($_POST['ten_kh'])){
		$post_save = array();
		$post_save['post_title'] = $_POST['ten_kh'] . "-" . $_POST['ten_sp'] ;
		$post_save['post_status'] = 'publish';
		$post_save['post_type'] = 'banhang';
		
		// Insert the post into the database
		$post_id = wp_insert_post( $post_save );
		add_post_meta( $post_id, 'ten_kh', $_POST['ten_kh'] );
		add_post_meta( $post_id, 'ten_sp', $_POST['ten_sp'] );
		add_post_meta( $post_id, 'ngayban', $_POST['ngayban'] );
		add_post_meta( $post_id, 'khuyenmai', $_POST['khuyenmai'] );
		add_post_meta( $post_id, 'dongia', $_POST['dongia'] );
		add_post_meta( $post_id, 'diachi', $_POST['diachi'] );
		add_post_meta( $post_id, 'soluong', $_POST['soluong'] );
		if($post_id) {
			?>
			<p class="success-meassage">Thêm thành công! </p>
		<?php
		}					
	}
	$args = array(
		...
		<table class="table table-condensed table-striped list-product">
			<thead>
				<tr>
					...
					<th class="col-sm-1">Khuyến mại</th>
					...
					<?php 
						if ($posts) {
							foreach ( $posts as $post ) { ?>
								<tr>
									<?php $postID = $post->ID; ?>
									<td><?php echo get_post_meta( $postID, 'ngayban', true ) ?></td>
									<td><?php echo get_post_meta( $postID, 'ten_kh', true ) ?></td>
									<td><?php echo get_post_meta( $postID, 'ten_sp', true ) ?></td>
									<td><?php echo get_post_meta( $postID, 'soluong', true ) ?></td>
									<td><?php echo get_post_meta( $postID, 'khuyenmai', true ) ?></td>
									<td><?php echo get_post_meta( $postID, 'dongia', true ) ?></td>
									<td><?php echo get_post_meta( $postID, 'diachi', true ) ?></td>
								</tr>
								<?php
							  }
...
add_action( 'wp_ajax_add_country', 'prefix_ajax_add_country' );
add_action( 'wp_ajax_nopriv_add_country', 'prefix_ajax_add_country' );

function prefix_ajax_add_country() {
	global $wpdb;
	if(!empty($_POST["keyword"])) {
		$query = "SELECT post_title FROM wp_posts WHERE post_title like '" . $_POST["keyword"] . "%' AND post_type='khachhang' ORDER BY post_title LIMIT 0,6 ";
		...
		?>
		<li><?php echo $country->post_title; ?></li>
		<?php } ?>
		</ul>
		<?php
		}
...
add_action( 'wp_ajax_kh_complete', 'prefix_ajax_kh_complete' );
add_action( 'wp_ajax_nopriv_kh_complete', 'prefix_ajax_kh_complete' );

function prefix_ajax_kh_complete() {
	global $wpdb;
	$data = array();
	$ten_kh = $_POST['ten_kh'];
	$query = "SELECT main.post_title as title, post.meta_value as value FROM wp_posts as main INNER JOIN wp_postmeta as post ON main.ID = post.post_id WHERE post_type = 'khachhang' AND post.meta_key = 'ma_kh' and  main.post_title LIKE '".strtoupper($ten_kh)."%'";
	$result = $wpdb->get_results($query);
	foreach ( $result as $post ) {
        $name = $post->title .'|'.$post->value;
...
add_action( 'wp_ajax_sp_complete', 'prefix_ajax_sp_complete' );
add_action( 'wp_ajax_nopriv_sp_complete', 'prefix_ajax_sp_complete' );

function prefix_ajax_sp_complete() {
	global $wpdb;
	$data = array();
	$ten_sp = $_POST['ten_sp'];
	$query = "SELECT main.post_title as title, post.meta_value as value FROM wp_posts as main INNER JOIN wp_postmeta as post ON main.ID = post.post_id WHERE post_type = 'sanpham' AND post.meta_key = 'ma_sp' and  main.post_title LIKE '".strtoupper($ten_sp)."%'";
	$result = $wpdb->get_results($query);
	foreach ( $result as $post ) {
        $name = $post->title .'|'.$post->value;
		array_push($data, $name);	
